    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-wrap gap-2 mb-8">
                {['Resumen General', 'Brechas', 'Nivel Estratégico', 'Nivel Táctico', 'Dimensiones', 'Análisis'].map((tab, idx) => (
                    <button key={tab} className={`px-6 py-2 text-sm font-semibold rounded-md transition-colors ${idx === 0 ? 'bg-gray-300 text-gray-800' : 'bg-gray-200 text-gray-600 hover:bg-gray-300'}`}>
                        {tab}
                    </button>
                ))}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <KPIcard
                    ...
                    subtitle={`Nivel de madurez: ${surveyData.globalAvg >= 4 ? 'Avanzado' : surveyData.globalAvg >= 3 ? 'Intermedio' : 'Inicial'}`}
